...
% Если вас тошнит от поля в 10мм --- увеличивайте до 20-ти, ну и про переплeт не забывайте:
...
% tcolorbox is used for code listings because mdframed with roundcorner
% relies on TikZ and TikZ-based mdframed frames cannot break across pages.
% tcolorbox with breakable=true supports both rounded corners and page breaks.
...
% Breakable code listing environment (non-float, page-breaking).
% Uses tcolorbox instead of mdframed: tcolorbox with breakable=true supports
% both rounded corners and page breaks. mdframed with roundcorner uses TikZ
% which cannot break across pages.
% Usage: \begin{codelisting}{Caption text}{label}
