...
\begin{codelisting}{Миграция 0003\_task\_completed.up.sql --- добавление признака выполнения}{listing:migration_completed}
  \begin{minted}{sql}
ALTER TABLE tasks ADD COLUMN IF NOT EXISTS is_completed BOOLEAN NOT NULL DEFAULT false;
  \end{minted}
\end{codelisting}

\begin{codelisting}{Миграция 0004\_task\_duration\_optional.up.sql --- опциональная длительность}{listing:migration_duration}
  \begin{minted}{sql}
ALTER TABLE tasks ALTER COLUMN duration_min DROP NOT NULL;
  \end{minted}
\end{codelisting}

\begin{codelisting}{Миграция 0005\_task\_datetime\_windows.up.sql --- раздельные дата и время окна}{listing:migration_datetime}
  \begin{minted}{sql}
