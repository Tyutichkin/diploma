...
func (a *NearestNeighborTW) Optimize(
    ...
    for len(order) < n {
        ...
        // Проход 2: запасной вариант — ближайший узел без учeта временного окна.
        if next == -1 {
            nextDurSec = math.MaxInt
            for i := 0; i < n; i++ {
                if visited[i] {
...
// запускает алгоритм оптимизации с учeтом временных окон
...
// optimizeRoute передаeт идентификаторы задач и предварительно вычисленную
