...
\begin{listing}[H]
  \caption{Алгоритм ближайшего соседа с временными окнами (nearest\_neighbor.go)}
  \label{app:lst_optimizer}
  \begin{mdframed}
    \begin{minted}{go}
package optimizer
...
\begin{listing}[H]
  \caption{HTTP-обработчики эндпоинтов маршрутов (handlers\_routes.go)}
  \label{app:lst_handlers_routes}
  \begin{mdframed}
    \begin{minted}{go}
package http
...
\begin{listing}[H]
  \caption{Ключевые функции API-клиента: авторизованные запросы и обновление токена (client.ts)}
  \label{app:lst_api_client}
  \begin{mdframed}
...
\begin{listing}[H]
  \caption{Построение матрицы расстояний через Яндекс Маршруты JS API (routeOptimizer.ts)}
  \label{app:lst_route_optimizer}
  \begin{mdframed}
    \begin{minted}{typescript}
