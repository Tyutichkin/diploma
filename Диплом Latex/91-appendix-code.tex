...
\begin{codelisting}{Алгоритм ближайшего соседа с временными окнами и просмотром вперед (\texttt{nearest\_neighbor.go})}{app:lst_optimizer}
    \begin{minted}{go}
package optimizer
...
\begin{codelisting}{HTTP-обработчик оптимизации маршрута (\texttt{handlers\_routes.go}, метод \texttt{Optimize})}{app:lst_handlers_routes}
    \begin{minted}{go}
...
// Если DistanceMatrix передана — бэкенд не ходит в OSRM
// и порядок согласован с Yandex.
...
\begin{codelisting}{Ключевые функции API-клиента: оптимизация маршрута и обновление токена (\texttt{client.ts})}{app:lst_api_client}
...
\begin{codelisting}{Построение матрицы расстояний через Яндекс Маршруты JS API (\texttt{routeOptimizer.ts})}{app:lst_route_optimizer}
    \begin{minted}{typescript}
            ...
            try {
              ...
              if (modelRoutes.length > 0) {
                ...
                resolve({
                  ...
                  durationSec:
                    totalDurSec > 0
                      ? roundUpSecToMinute(totalDurSec)
                      : fallbackSec,
                });
                return;
              }
