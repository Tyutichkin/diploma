...
function getMarkerLayout(): any {
    ...
    const template =
        `<div class="task-marker" style="box-sizing:border-box;display:flex;align-items:center;justify-content:center;` +
    ...
    cachedLayout = window.ymaps.templateLayoutFactory.createClass(template, {
        build: function (this: any) {
            cachedLayout.superclass.build.call(this);
        },
        getShape: function (this: any) {
            const offset: [number, number] = this.getData()?.options?.get(
                "iconOffset",
            ) ?? [-MARKER_RADIUS, -MARKER_RADIUS];
            const cx = offset[0] + MARKER_RADIUS;
            const cy = offset[1] + MARKER_RADIUS;
            return new window.ymaps.shape.Circle(
                new window.ymaps.geometry.pixel.Circle([cx, cy], MARKER_RADIUS),
            );
        },
    });
    return cachedLayout;
